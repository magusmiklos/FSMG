...
function setUpGame(cubes){
    ...
    //handle next map and refresh and stay on current map
    let mapId;
    ...
    for (let i = 0; i < cubes; i++) {
        ...
        if(map[x][y] === "I") c.classList.add('ice');
...
function tick(){
    ...
    for (let index = 0; index < childNodes.length; index++) {
        ...
        cubeMatrix.forEach(spot => {
            ...
            // Break the ice
            if(cube.classList.contains("ice")){
                if(spot.classList.contains("true")){
                    cube.classList.add("true");
                    cube.classList.remove("ice");
                    change = false;
                }
            }
        });
